...
try:
	...
	owner_SQL = "SELECT * FROM people p WHERE UPPER(p.sin) = '"+sin.upper()+"'"
	sowner_SQL = "SELECT * FROM people p WHERE UPPER(p.sin) = '"+ssin.upper()+"'"
	print owner_SQL
	oresult = SendToOracle(owner_SQL, False)
	sresult = SendToOracle(sowner_SQL, False)
	if oresult == []:
		...
		addperson_SQL = "INSERT INTO people VALUES ('"+ssin+"', '"+name+"',"+height+","+weight+",'"+eye_color+"','"+hair_color+"','"+addr+"','"+gender+"',TO_DATE('"+bday+"','YYYY-MM-DD'))"
		result = SendToOracle(addperson_SQL, True)
	print "success"
